...
const CourseData = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(`${url}/course/dataUpload` , formData, {
        headers: {
  ...
  return (
    <div>
      <div className="text-2xl font-semibold flex items-start lg:pl-20 sm:pl-2 pl-2">Course Data Upload</div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center lg:gap-8">
        <div className=" flex flex-col items-start">
          ...
          <label className=" text-blue-700 text-sm font-bold mb-2">Upload Course File </label>
          <input
      ...
      <div className='flex items-start justify-center pl-2 text-justify text-xs text-gray-400'>NOTE: The headers in the Excel file should be as follows !. =&gt; courseCode, courseName, courseWeek, duration</div>
